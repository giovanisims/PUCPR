    ...
    try:
        while True:
            ...
            if client_port == 10000:
                # The control panel sends messages in plain text
                message = raw_message.decode('utf-8')
                print(f"Received admin command from {address_str}")
                match message:
                    case "1":
                ...
                try:
                    # Decrypt the message
                    try:
                        decrypted_bytes = cipher.decrypt(raw_message)
                        message = decrypted_bytes.decode('utf-8')
                        print(f"Decrypted input from {address_str}: {message}")
